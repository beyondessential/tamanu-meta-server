...
Draft, waits on the source worker. Make a Kubernetes server's reachability Canopy-determined: green by default, failing only when the configured namespace no longer exists. Add the per-cluster connection/permissions self-alert (escalating) as a Canopy-wide check, and when a cluster is unreachable, mark its servers' pulled and harvested checks broken without raising them to failures, so an unreachable cluster surfaces as its one Canopy-wide failure rather than flooding the fleet. Verifies spec: K8S. Verifies spec: SELF. Verifies spec: CHK.
