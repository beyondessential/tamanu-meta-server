...
- [x] **Check identity: the namespace** — a check is identified by namespace and name, not name alone. Design settled; the checklist is in that section
...
- `devices` keeps its name, holding the identity half of the split. The machine came out of it (`machines.device_id`, and `DeviceRole::Server` became `DeviceRole::Machine` with `server` as an input alias), which is the separation the card asked for; renaming the remainder to `identities` would be a second sweep across the enrolment, certificate and authentication paths for a word. The specs say identity because that is what it is, and [DTR](../../specs/private-server/device-trust.md) is the place the two vocabularies meet.
...
- [x] **`reconcile_liveness` sees machine-subject checks again** — its refresh SQL filters `issues.application_id IS NOT NULL`, which was every check before this card split the target and is now only half of them. A machine-subject check files with `machine_id` and a null `application_id`, so its `last_seen` never refreshes: seven days after the split every one of them goes quiet and decommissions itself. The filter widens to either target being set. It excluded group- and canopy-wide filings, which the source filter beside it already excludes, so nothing else rides on it. Not namespace work — a regression from the machine split — but it lands in the function the catalog-gates item is opening anyway. The same filter was in `decommission`'s fleet-wide sweep, where it would have left a retired machine-subject check's states outstanding and still counting; that one is now a match on the namespace rather than on either target being set, since the sweep has to narrow to the entry being retired anyway
- [x] **`CheckPolicy` keyed by the namespace** — `live_cataloged_pairs` returns `HashSet<(String, Namespace, String)>`; `register`/upsert, `gone_quiet`'s ordering, and `decommission`'s fleet-wide sweep all take it. `decommission` is the one to watch: retiring `disk_free` must not retire another namespace's. `live_cataloged_pairs` returns a `CatalogKeys` set of `CatalogKey` triples, and the two identity filters (`catalog_identity`, `scoped_identity`) are the only places the namespace columns are compared, both on `IS NOT DISTINCT FROM` so a flat entry's nulls match
- [x] **Ingest registers with the namespace** — `public-server/src/statuses.rs:546` and `:623` already compute `CheckSubject::of`; they now carry it into the catalog registration alongside the reporting application's type. This is where the migration's rule and the ingest rule are locked to the same function, via `Namespace::for_application`
- [x] **The catalog gates widen** — 7 `live_cataloged_pairs` loads and 8 membership tests across `issues.rs` (1523, 1624, 1741, 1914, 1952, 3758), `check_policies.rs:886`, `private-server/src/fns/statuses.rs:928`. `health_from_check_state` takes `(id, group_id, type)` triples; every caller already holds the type on its `ServerInfo`, so no caller gains a query. `source_freshness` is the exception and gets one `(id, type)` map query, paid in `reconcile_liveness`
- [x] **Scoped resolution matches on namespace** — fleet catalog → group → target still applies in order, with each step matching the filing's namespace. A group- or machine-scoped row on an application-subject check now covers one type, not all of them
- [x] **Private API and SPA** — the catalog list and detail carry the namespace; `/healthchecks/:source/:check` gains a segment, with a redirect from the two-segment form so a bookmarked check page still lands; an entry presents as `<type>.<check>` for an application namespace and the bare name otherwise. Regenerate `private-web/openapi.json` and `api-types.ts`. The URL segment is `-` for flat, `machine`, or `application.<type>`, built only through `healthcheckPath`/`healthcheckSettingsPath` since check names are arbitrary strings. Two entries can now share a source and a name, so the catalog's React key takes the namespace as well; without it the two rows collided and reconciled into each other
- [x] **CHK lines 62–63 and 99** — the namespace as data, `<type>.<check>` as presentation, and the catalog key gaining the namespace. Three other lines went with them: `(source, check)` was the stated key for documentation and for both decommissioning windows, and now reads as the catalogued check. The silences section needed a criterion it did not have, since a silence names a check and so names its namespace, and a group silence has to say which type's check it quiets because a group spans several.
      `## Names` also gained the addressing rule the redirect implements: an address naming only a source and a check name resolves to the one entry it can mean, and asks where several share it
- [x] **Tests** — two types reporting one name are two entries with independent ceilings; a machine-subject name from a structured source is one entry, not one per type; a silence on one namespace leaves the other's state alone; the fan-out preserves the review stamp while re-deriving `last_seen`; a namespace first reporting after migration registers pending review. Playwright coverage for the catalog and check-detail changes
	- [x] Playwright coverage, as `e2e/check-namespace.spec.ts`: one name from two types is two entries graded apart, a machine-subject name is one entry whatever reports it, a curated source's check keeps its bare name, a two-segment link lands on the one entry it means, an ambiguous one asks which check was meant, and a group silence on one type leaves the other type's check alerting
	- [x] The e2e seeds write the namespace ingest would have written. They were not merely stale: `seed.ts` and `seedIssue` upserted on `(source, check_name)`, an index the migration drops, so every seed touching the catalog would have raised on the missing conflict target. `seed.ts` now derives the namespace the way `Namespace::of` does, looking the reporting application's type up from its own row, and a Rust test beside `MACHINE_SUBJECT_CHECKS` parses the TypeScript snapshot of that list to hold the two together
	- [x] The Rust half, as `tests/it/check_namespace.rs`: a group silence on one type leaves the other type's check alerting, the migration's fan-out is replayed for real (revert, seed the pre-split catalog and states, re-apply) and carries the grading and the review while re-deriving `last_seen` per namespace, and a type reporting a name for the first time lands unreviewed at the default ceiling however well its siblings are graded.
	      The pending-review case goes through `upsert_default` rather than `file_check`: the device push is the only path that registers unreviewed, since Canopy's own filings register already vetted
