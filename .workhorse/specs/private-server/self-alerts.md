...
- Canopy cannot reach one or more registered Kubernetes clusters, or lacks the permissions to read them (escalating); the affected clusters are named in its detail (see [K8S](../monitoring/kubernetes.md)). It clears when every registered cluster is reachable again.
