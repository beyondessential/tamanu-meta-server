...
- **source** — the name of the reporter pushing this status.
  ...
  The `alertd` source is also populated by a cluster's relay for the Kubernetes servers it harvests, so a device push is one of two origins for it (see [K8S](../monitoring/kubernetes.md)).
