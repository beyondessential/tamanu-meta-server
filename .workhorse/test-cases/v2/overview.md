...
- [x] A reporter field named `source`, `health`, `check` or `result` inside `detail` is recorded and does not collide with the envelope (verifies spec: STA)
- [x] Two applications sharing a key cannot be expressed in a payload (verifies spec: STA)
...
- [x] Both detail pages end with the group's tree, with the current page highlighted
...
- [x] Neither detail page shows a status dot beside its title
...
- [x] The fleet listing has no ungrouped tab
