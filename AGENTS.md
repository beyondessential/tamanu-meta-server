...
- **Say group, environment, rank — never "deployment".** A group is what Canopy attaches shared state to (see `.workhorse/specs/servers/groups.md`); a group's servers at one rank are one of its environments; an installation of Canopy itself is "the Canopy instance". The word survives only in the `billing.deployment` cost-allocation label, which is read by AWS cost allocation and by devices reading their effective tags, so `grep -rin deployment` over specs, crates, and `private-web/src` should turn up only that label and Kubernetes `Deployment` resources.
