...
- **The admin identity has a debug-only shortcut.** In debug builds the `TailscaleUser` / `TailscaleAdmin` extractors act as a fixed `admin@localhost` administrator without reading any headers, so ordinary tests are admin for free (existing tests rely on this — don't disturb it). Set `CANOPY_TRUST_TAILSCALE_HEADERS=1` to authenticate from the real `Tailscale-User-Login` / `-Name` / `-Profile-Pic` headers instead, resolving admin status through the real allow-list and tailnet policy. Use it to exercise a denial (`tests/it/admin_auth.rs` is the pattern: set the env var, send headers per request, seed the allow-list with `database::admins::Admin::add`). The shortcut is compiled out of release builds, so no misconfiguration can bypass authentication in production.
