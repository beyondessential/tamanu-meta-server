...
async fn same_check_name_is_distinct_per_source() {
	commons_tests::db::TestDb::run(async |mut conn, _| {
		...
		CheckPolicy::upsert_default(&mut conn, "seedling", &app_ns(), "disk_space")
			.await
		...
		let seedling = CheckPolicy::get(&mut conn, "seedling", &app_ns(), "disk_space")
			.await
			.expect("get")
...
async fn apply_caps_at_ceiling_or_defaults_to_warning() {
	commons_tests::db::TestDb::run(async |mut conn, _| {
		...
		CheckPolicy::upsert_default(&mut conn, "alertd", &app_ns(), "cert_expiry")
			.await
			.expect("seed");
		CheckPolicy::update(
			&mut conn,
...
async fn register_marks_canopy_checks_already_reviewed() {
	commons_tests::db::TestDb::run(async |mut conn, _| {
		...
		let row = CheckPolicy::get(&mut conn, "canopy", &Namespace::Flat, "backup_stale")
			.await
			.expect("get")
			.expect("row exists");
...
async fn escalates_only_sticks_at_a_failed_ceiling() {
	commons_tests::db::TestDb::run(async |mut conn, _| {
		...
		.await
		.expect("register");
		let registered = CheckPolicy::get(&mut conn, "canopy", &Namespace::Flat, "warn_only")
			.await
			.expect("get")
			.expect("row exists");
		assert!(
...
async fn update_stamps_review_metadata_even_on_no_op_save() {
	commons_tests::db::TestDb::run(async |mut conn, _| {
		CheckPolicy::upsert_default(&mut conn, "alertd", &app_ns(), "noisy_check")
			.await
			.expect("seed");
...
async fn list_orders_by_source_then_check_name() {
	commons_tests::db::TestDb::run(async |mut conn, _| {
		for (source, name) in [("alertd", "zeta"), ("alertd", "alpha"), ("seedling", "mu")] {
			CheckPolicy::upsert_default(&mut conn, source, &app_ns(), name)
				.await
				.expect("seed");
		}
		let rows = CheckPolicy::list(&mut conn).await.expect("list");
