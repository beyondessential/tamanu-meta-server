...
async fn insert_grouped_server(conn: &mut diesel_async::AsyncPgConnection) -> Uuid {
	...
	let server: RowId = sql_query(
		"WITH m AS (INSERT INTO machines (group_id) VALUES ($1) RETURNING id) INSERT INTO applications (host, group_id, machine_id) SELECT 'http://reeval.invalid/', $1, m.id FROM m RETURNING id",
	)
