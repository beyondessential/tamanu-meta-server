...
async fn restore_window_opens_for_a_day_then_closes() {
	commons_tests::db::TestDb::run(|mut conn, _url| async move {
		...
		let until = Application::allow_restore(&mut conn, created.id, Some("op@example"))
			.await
			.unwrap();
		...
		let reloaded = Application::get_by_id(&mut conn, created.id).await.unwrap();
		...
		Application::disallow_restore(&mut conn, created.id)
			.await
			.unwrap();
		let reloaded = Application::get_by_id(&mut conn, created.id).await.unwrap();
...
async fn expired_window_reads_as_closed() {
	commons_tests::db::TestDb::run(|mut conn, _url| async move {
		...
		let created = Application::create(&mut conn, server).await.unwrap();
		assert!(
