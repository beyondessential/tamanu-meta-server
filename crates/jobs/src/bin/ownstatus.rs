...
use database::{Db, applications::Application, statuses::NewStatus};
...
async fn record_own_status(pool: Db, start: Instant) -> Result<()> {
	...
	let server = Application::own(&mut db).await?;
...
pub fn spawn() -> JoinHandle<()> {
	...
	task::spawn(async move {
		loop {
			...
			// `Application::own` row) spins as fast as the query can fail, burning
