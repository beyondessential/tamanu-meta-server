...
async fn status_missing_auth_headers() {
	commons_tests::server::run(async |mut conn, public, _| {
		...
		conn.batch_execute(
			"WITH m AS (INSERT INTO machines (id) VALUES ('11111111-1111-1111-1111-111111111111') RETURNING id) INSERT INTO applications (id, host, kind, machine_id) VALUES ('11111111-1111-1111-1111-111111111111', 'https://test.com', 'facility', '11111111-1111-1111-1111-111111111111')",
		)
		.await
		.unwrap();
...
async fn malformed_json_body() {
	commons_tests::server::run(async |mut conn, public, _| {
		...
		conn.batch_execute(
			"WITH m AS (INSERT INTO machines (id) VALUES ('11111111-1111-1111-1111-111111111111') RETURNING id) INSERT INTO applications (id, host, kind, machine_id) VALUES ('11111111-1111-1111-1111-111111111111', 'https://test.com', 'central', '11111111-1111-1111-1111-111111111111')",
		)
		.await
		.unwrap();
