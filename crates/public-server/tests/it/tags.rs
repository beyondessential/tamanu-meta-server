...
async fn tags_endpoint_returns_server_tags_when_ungrouped() {
	commons_tests::server::run_with_device_auth(
		...
		async |mut conn, cert, device_id, public, _| {
			let server_id = Uuid::new_v4();
			sql_query(
				"WITH m AS (INSERT INTO machines (id) VALUES ($1) RETURNING id) INSERT INTO applications (id, host, type, device_id, tags, machine_id) \
				 VALUES ($1, 'https://lone.example.com', 'tamanu-central', $2, '{\"role\": \"primary\"}'::jsonb, $1)",
			)
			.bind::<sql_types::Uuid, _>(server_id)
			.bind::<sql_types::Uuid, _>(device_id)
			.execute(&mut conn)
	...
	commons_tests::server::run_with_device_auth(
		...
		async |mut conn, cert, device_id, public, _| {
			...
			sql_query("INSERT INTO server_groups (id, name) VALUES ($1, 'synthetic-cluster')")
				...
				.unwrap();
			sql_query(
				"WITH m AS (INSERT INTO machines (id, group_id) VALUES ($1, $3) RETURNING id) INSERT INTO applications (id, host, type, rank, device_id, group_id, machine_id) \
				 VALUES ($1, 'https://s.example.com', 'tamanu-facility', 'production', $2, $3, $1)",
			)
			.bind::<sql_types::Uuid, _>(server_id)
			.bind::<sql_types::Uuid, _>(device_id)
			.bind::<sql_types::Uuid, _>(group_id)
...
async fn tags_endpoint_includes_effective_billing_labels() {
	commons_tests::server::run_with_device_auth(
		...
		async |mut conn, cert, device_id, public, _| {
			...
			sql_query(
				"WITH m AS (INSERT INTO machines (id, group_id) VALUES ($1, $3) RETURNING id) INSERT INTO applications (id, host, type, rank, device_id, group_id, machine_id) \
				 VALUES ($1, 'https://b.example.com', 'tamanu-central', 'production', $2, $3, $1)",
			)
			.bind::<sql_types::Uuid, _>(server_id)
			.bind::<sql_types::Uuid, _>(device_id)
			.bind::<sql_types::Uuid, _>(group_id)
...
async fn tags_endpoint_server_billing_tags_win() {
	commons_tests::server::run_with_device_auth(
		...
		async |mut conn, cert, device_id, public, _| {
			...
			.unwrap();
			sql_query(
				"WITH m AS (INSERT INTO machines (id, group_id) VALUES ($1, $3) RETURNING id) INSERT INTO applications (id, host, type, rank, device_id, group_id, tags, machine_id) \
				 VALUES ($1, 'https://ov.example.com', 'tamanu-central', 'production', $2, $3, \
...
async fn tags_endpoint_group_billing_tags_win_over_defaults() {
	commons_tests::server::run_with_device_auth(
		...
		async |mut conn, cert, device_id, public, _| {
			...
			sql_query(
				"WITH m AS (INSERT INTO machines (id, group_id) VALUES ($1, $3) RETURNING id) INSERT INTO applications (id, host, type, rank, device_id, group_id, machine_id) \
				 VALUES ($1, 'https://gs.example.com', 'tamanu-central', 'production', $2, $3, $1)",
			)
			.bind::<sql_types::Uuid, _>(server_id)
			.bind::<sql_types::Uuid, _>(device_id)
			.bind::<sql_types::Uuid, _>(group_id)
...
async fn tags_endpoint_billing_stage_is_per_server_rank() {
	commons_tests::server::run_with_device_auth(
		...
		async |mut conn, cert, device_id, public, _| {
			...
			sql_query(
				"WITH m AS (INSERT INTO machines (id, group_id) VALUES ($1, $3) RETURNING id) INSERT INTO applications (id, host, type, rank, device_id, group_id, machine_id) \
				 VALUES ($1, 'https://clone.example.com', 'tamanu-central', 'clone', $2, $3, $1)",
			)
...
async fn tags_endpoint_billing_product_is_the_servers_own() {
	commons_tests::server::run_with_device_auth(
		...
		async |mut conn, cert, device_id, public, _| {
			...
			sql_query(
				"WITH m AS (INSERT INTO machines (id, group_id) VALUES ($1, $3) RETURNING id) INSERT INTO applications (id, host, type, rank, device_id, group_id, machine_id) \
				 VALUES ($1, 'https://lims.example.com', 'senaite', 'clone', $2, $3, $1)",
			)
			.bind::<sql_types::Uuid, _>(server_id)
			.bind::<sql_types::Uuid, _>(device_id)
			.bind::<sql_types::Uuid, _>(group_id)
