-- Re-add the error column to statuses table
ALTER TABLE statuses ADD COLUMN error TEXT;

-- Recreate the latest_statuses view with error/success logic
CREATE OR REPLACE VIEW latest_statuses AS (
	...
	select
		...
		COALESCE(((errors.id IS NULL AND successes.id IS NOT NULL) OR (successes.created_at > errors.created_at) AND successes.created_at > (current_timestamp - '5 minutes'::interval)), FALSE) as is_up,
		CASE
			WHEN (successes.id IS NULL AND errors.latency_ms >= 10000) THEN NULL
...
-- Recreate indexes that reference the error field
