-- Create partition management functions for statuses table
--
...
-- Usage: SELECT create_statuses_partitions(8);
...
-- Usage: SELECT drop_old_statuses_partitions(520);  -- Drop partitions older than 10 years
CREATE OR REPLACE FUNCTION drop_old_statuses_partitions(weeks_to_keep INTEGER DEFAULT 520)
...
BEGIN
    ...
    FOR v_partition_record IN
        SELECT tablename
        FROM pg_tables
        WHERE tablename ~ '^statuses_\d{4}w\d{2}$'
        AND schemaname = 'public'
    LOOP
        -- Extract year and week from partition name
        v_partition_year := SUBSTRING(v_partition_record.tablename FROM 'statuses_(\d{4})w')::INT;
...
-- Usage: SELECT * FROM get_statuses_partition_info();
CREATE OR REPLACE FUNCTION get_statuses_partition_info()
RETURNS TABLE(
    partition_name TEXT,
    row_count BIGINT,
    total_size TEXT,
...
BEGIN
    ...
    WHERE t.tablename ~ '^statuses_\d{4}w\d{2}$'
...
COMMENT ON FUNCTION create_statuses_partitions(INTEGER) IS
    'Creates the next N weeks of partitions for the statuses table. Run weekly via cron: SELECT create_statuses_partitions(8);';

COMMENT ON FUNCTION drop_old_statuses_partitions(INTEGER) IS
    'Drops partitions older than N weeks. Default is 520 weeks (10 years). Run monthly via cron: SELECT drop_old_statuses_partitions();';

COMMENT ON FUNCTION get_statuses_partition_info() IS
    'Returns detailed information about all statuses partitions including sizes, row counts, and date ranges.';
