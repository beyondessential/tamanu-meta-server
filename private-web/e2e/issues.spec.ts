...
// per-server link (`/applications/null`) or a per-server silence action — both only
...
test.describe("group-scoped issue rendering", () => {
	...
	test("a group-scoped issue has no /applications/null link", async ({
		page,
		sql,
	}) => {
		...
		await expect(page.locator('a[href="/applications/null"]')).toHaveCount(0);
	});
